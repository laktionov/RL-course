...
class RunningMeanVar:
    ...
    def update(self, batch):
        ...
        if not batch.shape[1:] == self.mean.shape:
            raise ValueError(
                f"batch has invalid shape: {batch.shape}, "
...
class Normalize(gymnasium.Wrapper):
    """
    ...
    def __init__(
        ...
        super().__init__(env)
        self.obs_rmv = (
            RunningMeanVar(shape=self.observation_space.shape) if obs else None
        )
    ...
    def observation(self, obs):
        ...
        if not self.obs_rmv:
            return obs
        rmv_batch = (
            np.expand_dims(obs, 0) if not hasattr(self.env.unwrapped, "nenvs") else obs
        )
    ...
    def step(self, action):
        ...
        if self.ret_rmv:
            ...
            rews = np.clip(
                rews / np.sqrt(self.ret_rmv.var + self.eps), -self.cliprew, self.cliprew
            )
        self.ret[np.logical_or(terminated, truncated)] = 0.0
